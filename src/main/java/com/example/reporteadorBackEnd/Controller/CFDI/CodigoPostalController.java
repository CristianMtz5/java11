...
public class CodigoPostalController {
    ...
    @GetMapping("/conAsentamientos/{id}")
    public List<CodigoPostalDTO> byIdConAsentamientos(@PathVariable("id") String id) {
        // List<AsentamientosDTO> algo = asentamientosService.getByIdCodigoPostal(id);
        List<CodigoPostalDTO> codigoPostalDTO = codigoPostalService.getByIdConAsentamientos(id);
        return codigoPostalDTO;
    }

    @GetMapping("/byId/{id}")
    public List<GlobalDTO> byId(@PathVariable("id") String id) {
        return (List<GlobalDTO>) codigoPostalService.getById(id);
    }

    @GetMapping("/getAll/{status}")
    public List<GlobalDTO> allByStatusDTO(@PathVariable("status") Boolean status, Sort sort) {
        return (List<GlobalDTO>) codigoPostalService.getAll(status, sort);
    }
