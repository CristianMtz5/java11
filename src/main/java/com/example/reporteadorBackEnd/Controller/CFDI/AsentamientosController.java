...
public class AsentamientosController {
    ...
    @GetMapping("/byCodigoPostal/{id}")
    public List<AsentamientosDTO> byIdCodigoPostal(@PathVariable("id") String id) {
        return (List<AsentamientosDTO>) asentamientosService.getByIdCodigoPostal(id);
    }
    ...
    public List<GlobalDTO> allByStatusDTO(@PathVariable("status") Boolean status, Sort sort) {
        return (List<GlobalDTO>) asentamientosService.getAll(status, sort);
    }
