...
public class TipoRelacionService {
    ...
    public List<TipoRelacionEntity> getAllTipoRelacionByStatus(Boolean status, Sort sort){
        ...
        > tipoRelacion = tipoRelacionRepository.findByStatus(status, sort);
        return tipoRelacion;
    }

    public List<GlobalDTO> getAll(Boolean status, Sort sort) {
        sort = Sort.by("id");
        List<TipoRelacionEntity> tipoRelacion = tipoRelacionRepository.findByStatus(status, sort);
        return tipoRelacion.stream()
                .map(this::entityToDTO)
                .collect(Collectors.toList());
    }

    public GlobalDTO entityToDTO(TipoRelacionEntity tipoRelacion) {
        return new GlobalDTO(tipoRelacion);
    }
}
